...
> {
  ...
  private actionsContainer = React.createRef<HTMLDivElement>();
  ...
  render() {
    ...
    const rightMarkup = rightPopoverableActions ? (
      <div className={styles.RightContainer} testID="FilterShortcutContainer">
        {popoverFrom(this.getActionsToRender())}
      </div>
    ) : null;
    ...
    const filtersActionMarkup = filtersAction && (
      <div className={styles.ActionsInnerContainer}>{filtersAction}</div>
    );

    const sortActionMarkup = sortAction && (
      <div className={styles.ActionsInnerContainer}>{sortAction}</div>
    );

    const actionsMarkup =
      filtersAction || sortAction ? (
        <div className={styles.ActionsContainer} ref={this.actionsContainer}>
          {filtersActionMarkup}
          {sortActionMarkup}
        </div>
      ) : null;
  ...
  private measureAvailableWidth() {
    if (this.container.current && this.moreFiltersButtonContainer.current) {
      ...
      const moreFiltersButtonWidth = this.moreFiltersButtonContainer.current.getBoundingClientRect()
        .width;
      let filtersActionWidth = 0;

      if (this.props.filtersAction && this.actionsContainer.current) {
        filtersActionWidth = this.actionsContainer.current.getBoundingClientRect()
          .width;
      }
  ...
  private getActionsToRender(): PopoverableAction[] {
    let remainingWidth = this.state.availableWidth;
    const {rightPopoverableActions} = this.props;
    const actionsToReturn: PopoverableAction[] = [];
    const actions = rightPopoverableActions ? rightPopoverableActions : [];
...
function activatorButtonFrom(action: BaseAction): React.ReactElement<any> {
  return (
    <Button
      ...
      disclosure
      testID={`#{action.key}ShortcutToggleButton`}
    >
      {action.content}
    </Button>
  );
